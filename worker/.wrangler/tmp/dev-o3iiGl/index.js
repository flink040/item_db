...
// .wrangler/tmp/bundle-qWWyp9/middleware-loader.entry.ts
init_modules_watch_stub();

// .wrangler/tmp/bundle-qWWyp9/middleware-insertion-facade.js
init_modules_watch_stub();
...
var integerFromUnknown = /* @__PURE__ */ __name((opts = {}) => external_exports.preprocess((value) => {
  ...
  let schema = external_exports.number().int();
  if (opts.positive) {
...
var itemSchema = external_exports.object({
  ...
  if (!resolvedName) {
    ctx.addIssue({
      ...
      path: ["name"],
      message: "Name (oder Titel) wird ben\xF6tigt."
    });
  }
...
__name(normaliseItemPayload, "normaliseItemPayload");
async function validateEnchantments(client, enchantments) {
  if (!enchantments.length) {
    return [];
  }
  const uniqueIds = /* @__PURE__ */ new Map();
  ...
  const { data, error } = await client.from("enchantments").select("id,max_level").in("id", Array.from(uniqueIds.keys()));
  if (error) {
  ...
  if (violations.length) {
    throw Object.assign(
      new Error(
        violations.map((entry) => `Verzauberung ${entry.id}: Level ${entry.received} > erlaubt ${entry.max}`).join(", ")
      ),
      {
        status: 400,
...
__name(validateEnchantments, "validateEnchantments");
async function insertItemWithEnchantments(client, item, enchantments) {
  const resolvedStars = typeof item.star_level === "number" ? item.star_level : typeof item.stars === "number" ? item.stars : 0;
  const buildPayloadVariant = /* @__PURE__ */ __name((starColumn, useLegacyFallback) => {
    const payload = {
  ...
  const executeInsert = /* @__PURE__ */ __name(async (starColumn, useLegacyFallback) => client.from("items").insert(buildPayloadVariant(starColumn, useLegacyFallback)).select().single(), "executeInsert");
  ...
  for (const starColumn of starColumns) {
    ...
    const missingStarColumn = message.includes('column "stars"') || message.includes("column items.stars");
  ...
  const derivedStars = typeof insertedItem.stars === "number" ? insertedItem.stars : typeof insertedItem.star_level === "number" ? insertedItem.star_level : resolvedStars;
  ...
  const { error: enchantError, data: enchantRows } = await client.from("item_enchantments").insert(enchantPayload);
  if (enchantError) {
...
__name(insertItemWithEnchantments, "insertItemWithEnchantments");
var app = new Hono2();
...
app.post("/api/items", async (c) => {
  ...
  try {
    ...
    if (error && typeof error === "object" && "status" in error && typeof error.status === "number") {
      ...
      const message2 = error instanceof Error ? error.message : "Speichern fehlgeschlagen.";
      return c.json({ error: "validation", message: message2 }, status);
    }
